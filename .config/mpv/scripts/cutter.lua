...
local o = {
  -- ffmpeg_path = "flatpak-spawn",
  ffmpeg_path = "ffmpeg",
  target_dir = "~/usr/pics/mpv/cuts",
...
local function cut(shift, endpos)
  ...
  -- sel_sub = for _, track in iparis(tracks) do
  --   if track["selected"] == true then
  --     if track["type"]
  --     end
  -- end
  -- cmds:arg("-lavfi", string.format("subtitles='%s':si=%d",
  --                                  utils.join_path(
  --                                    mp.get_property("working-directory", ""),
  --                                    mp.get_property("path")
  --                                  ),
  --                                  (sel_sub == nil and 0 or sel_sub["id"] - 1)))
...
mp.add_key_binding("Ctrl+k", "slicing_mark", toggle_mark)
mp.add_key_binding("Ctrl+a", "slicing_audio", toggle_audio)
mp.add_key_binding("Ctrl+K", "clear_slicing_mark", clear_toggle_mark)
